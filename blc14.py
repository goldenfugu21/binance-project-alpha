...
def calculate_target_price(
        entry_price: Decimal, leverage: Decimal, target_roi_percent: Decimal, position_type: str, 
        entry_fee_rate: Decimal, exit_fee_rate: Decimal # 두 가지 수수료율 인수로 변경
) -> Decimal:
    target_roi = target_roi_percent / Decimal('100.0')
    if position_type.lower() == 'long':
        # P_target = P_entry * (1 + (ROI/L) + Fee_Entry) / (1 - Fee_Exit)
        return entry_price * (Decimal('1') + (target_roi / leverage) + entry_fee_rate) / (Decimal('1') - exit_fee_rate)
...
class BinanceCalculatorApp(QWidget):
    def __init__(self):
        ...
        # --- 기존 nPNL_Target 변수 유지 (사용하지 않음, 호환성 목적) ---
        self.current_position_nPNL_Target = Decimal('0') 
        # ----------------------------------------------------
    ...
    def initUI(self):
        ...
        fee_type_layout.addWidget(QLabel("수수료 종류:"))
    ...
    def update_position_status(self):
        try:
            ...
            # --- Margin 및 Net Target Price 계산 기준 설정 ---
            first_p = open_positions[0] 
            entry_price_for_calc = Decimal(first_p['entryPrice'])
            position_amt_abs_for_calc = Decimal(first_p['positionAmt']).copy_abs()
            
            leverage = self.leverage_input.text()
            roi_percent = self.roi_input.text()
            
            if not leverage or Decimal(leverage) <= Decimal('0'):
                leverage = Decimal('1') 
            else:
                leverage = Decimal(leverage)
            
            # ROI 입력값이 비어있거나 0 이하면 마스터가 설정한 최소치(0.01)를 사용
            if not roi_percent or Decimal(roi_percent) <= Decimal('0'):
                roi_percent = Decimal('0.01') 
            else:
                roi_percent = Decimal(roi_percent)
                
            # Margin = (API Entry Price * API Position Amount) / UI Leverage
            margin = (entry_price_for_calc * position_amt_abs_for_calc) / leverage
            
            # 수수료율 설정 (Target Price 계산용: 진입=Taker, 청산=Maker)
            entry_fee_rate = Decimal(self.config.get('TRADING', 'taker_fee_rate'))
            exit_fee_rate = Decimal(self.config.get('TRADING', 'maker_fee_rate'))
            position_side_type = "long" if Decimal(first_p['positionAmt']) > 0 else "short"
            
            # Net Target Price 계산 (API Entry Price 기준, Taker/Maker Fee 분리 적용)
            net_target_price = calculate_target_price(
                entry_price_for_calc, leverage, roi_percent, position_side_type, 
                entry_fee_rate, exit_fee_rate # 수정된 인수로 전달
            )
            # Tick Size에 맞게 조정 (청산 가격이므로 CEILING/FLOOR 적용)
            if position_side_type == 'long':
                net_target_price_adj = net_target_price.quantize(self.tick_size, rounding=ROUND_CEILING)
            else:
                net_target_price_adj = net_target_price.quantize(self.tick_size, rounding=ROUND_FLOOR)
            # ----------------------------------------------------------------
            ...
            for p in open_positions:
                ...
                # --- xROE (순수 수익률) 계산: nPNL / Margin * 100 ---
                xroe_text = "N/A"
                xroe_color = "black"
                
                # Margin이 0에 가까운지 확인 (안전 가드)
                if abs(margin) > Decimal('0.000001'):
                    # xROE = nPNL / Margin * 100
                    xroe_percent = (net_pnl / margin) * Decimal('100')
                    xroe_text = f"{xroe_percent:,.2f}%" 
                    
                    # xROE 색상 설정: 순수익률 기준
                    if xroe_percent >= Decimal('0'):
                        xroe_color = "green" if xroe_percent > Decimal('0') else "black" # 0% 이상
                    else:
                        xroe_color = "red" # 손실
                else:
                    xroe_text = "MARGIN 0" 
                ...
                display_text += (f"<b style='font-size:11pt;'>{p['symbol']} <span style='color:{position_color};'>({position_side})</span></b><br>"
                                ...
                                f" - <b>순수익률(xROE):</b> <span style='color:{xroe_color};'><b>{xroe_text}</b></span><br>"
                                f" - <b>Net Target Price:</b> <span style='color:green;'><b>${net_target_price_adj:{price_format}}</b></span><br>" # Net Target Price 추가
    ...
    def update_quantity_from_slider(self):
        try:
            ...
            # 슬라이더 이동 시 Target Price와 NLV를 재계산하도록 트리거
            self.calculate_and_display_target() 
            
            # UI 입력값(ROI, 레버리지)이 변경될 수 있으므로 포지션 상태도 갱신하여 xROE에 반영
            self.update_position_status()
    ...
    def calculate_and_display_target(self):
        try:
            if not all([self.entry_price_input.text(), self.leverage_input.text(), self.roi_input.text(), self.position_type]):
            ...
            # Target Price 계산 시 Fee Rate를 설정
            # 계산기 패널의 Target Price는 UI의 Fee Type에 따라 계산됨
            if self.taker_radio.isChecked():
                exit_fee_rate = Decimal(self.config.get('TRADING', 'taker_fee_rate'))
                entry_fee_rate = Decimal(self.config.get('TRADING', 'taker_fee_rate'))
            else:
                exit_fee_rate = Decimal(self.config.get('TRADING', 'maker_fee_rate'))
                entry_fee_rate = Decimal(self.config.get('TRADING', 'maker_fee_rate'))
            ...
            # Target Price 계산 (기존 로직 유지)
            target_price = calculate_target_price(entry_price, leverage, target_roi_percent, self.position_type,
                                                     entry_fee_rate, exit_fee_rate) # 두 개의 Fee Rate 인수로 변경

            if self.tick_size > Decimal('0'):
                # 롱 포지션: 청산 가격은 올려서 (ROUND_CEILING) 유리하게 체결
                if self.position_type == 'long':
                    rounding_mode = ROUND_CEILING
                # 숏 포지션: 청산 가격은 내려서 (ROUND_FLOOR) 유리하게 체결
                else: 
            ...
            # NLV (Net Leverage Value) 계산: 목표 수익률 / 레버리지 + 왕복 수수료
            # NLV 계산 시에는 Taker/Maker 구분이 복잡하므로, 일단 Taker 기준으로 단순화하여 표시
            taker_fee_rate_cfg = Decimal(self.config.get('TRADING', 'taker_fee_rate'))
            required_change_percent = (target_roi_percent / leverage) + (taker_fee_rate_cfg * Decimal('200')) # 왕복 Taker 기준
            if self.position_type == 'long':
                color = "red"
                sign = "+"
            else:
            ...
            # 여기서 self.update_position_status()를 호출하여 xROE 및 Net Target Price를 갱신
            self.update_position_status()
            
        except Exception as e:
            logging.error(f"목표 가격/NLV 계산/표시 오류: {e}", exc_info=True)
            self.target_price_label.setText("Target Price: N/A")
            self.price_change_label.setText("NLV: N/A")
